...
class FasterWhisperConfig:
    ...
    language: Optional[str] = None  # Target language (None for auto-detection)
    model_path: Optional[str] = None  # Custom model path (None for default cache)
...
class AudioConfig:
    ...
    device_index: Optional[int] = None  # Specific device index (None for default)
...
class SystemConfig:
    ...
    log_file: Optional[Path] = None  # Log file path (None for stdout only)
...
class Config:
    ...
    def validate(self) -> None:
        ...
        if self.faster_whisper.model not in valid_models:
            raise ValueError(
                f"Invalid model: {self.faster_whisper.model}. "
        ...
        if self.faster_whisper.device not in valid_devices:
            raise ValueError(
                f"Invalid device: {self.faster_whisper.device}. "
        ...
        if self.audio.chunk_duration_ms <= 0:
            raise ValueError(
                f"chunk_duration_ms must be > 0, got {self.audio.chunk_duration_ms}"
            )

        # Validate VAD threshold
        if not 0.0 <= self.vad.threshold <= 1.0:
            raise ValueError(
                f"VAD threshold must be between 0.0 and 1.0, got {self.vad.threshold}"
            )
        ...
        if self.system.log_level.upper() not in valid_log_levels:
            raise ValueError(
                f"Invalid log_level: {self.system.log_level}. "
    ...
    def merge_with_overrides(self, overrides: dict[str, any]) -> "Config":  # type: ignore[valid-type]
        ...
        new_config = Config(
            faster_whisper=FasterWhisperConfig(**{
                **vars(self.faster_whisper),
                **overrides.get("faster_whisper", {})
            }),
            audio=AudioConfig(**{
                **vars(self.audio),
                **overrides.get("audio", {})
